...
def plot(config):
    ...
    for day in range(num_days):
        ...
        # True output data
        true_temp, true_salt, true_u, true_v, true_height = load_ocean_data(data_file, input_day+1+day, test=True)
    ...
    for var_idx, var in enumerate(variables):
        ...
        plot_variable_auto_alternate(true_data, pred_data, lon, lat, var, cmaps[var_idx], time, num_days, time_list)
        
    plot_velocity_auto_alternative(true_velocity_data, pred_velocity_data, lon, lat, time, num_days, time_list)
